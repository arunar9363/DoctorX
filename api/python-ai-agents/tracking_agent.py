...
def get_report_extraction_agent():
    """
    NEW AGENT: Specialized agent for extracting structured data from scanned health reports
    Handles lab reports, BP logs, glucose monitoring sheets, etc.
    """
    ...
    return Agent(
        ...
        description="Expert AI for extracting structured health data from scanned medical reports, lab results, and monitoring logs.",
        
        instructions=[
            "Role: You are a medical data extraction specialist.",
            "Your task is to analyze scanned health reports and extract vital signs data into a structured JSON format.",
            "",
            "### SUPPORTED REPORT TYPES:",
            "1. Blood Pressure Monitoring Logs",
            "2. Blood Glucose (Sugar) Reports",
            "3. Complete Blood Count (CBC) Reports",
            "4. Lipid Profile Reports",
            "5. Thyroid Function Tests (TSH, T3, T4)",
            "6. General Lab Reports with vital signs",
            "7. Home Monitoring Sheets",
            "",
            "### EXTRACTION RULES:",
            "",
            "**For Blood Pressure:**",
            "- Extract: Date/Time, Systolic value, Diastolic value, Pulse (if available)",
            "- Look for patterns like: 120/80, BP: 130/85, Systolic 125 Diastolic 82",
            "- Context clues: Morning, Evening, Before medication, After exercise",
            "",
            "**For Blood Glucose:**",
            "- Extract: Date/Time, Glucose value, Unit (mg/dL or mmol/L)",
            "- Context: Fasting (FBS), Post-meal (PPBS), Random (RBS)",
            "- Look for: Fasting Blood Sugar, Post Prandial, HbA1c",
            "",
            "**For Lab Reports:**",
            "- Extract: Test name, Value, Unit, Normal range",
            "- Common tests: Hemoglobin, RBC, WBC, Platelets, Cholesterol, Triglycerides",
            "",
            "**For Dates:**",
            "- Convert to ISO format: YYYY-MM-DDTHH:MM:SS",
            "- If only date is available, use T00:00:00",
            "- If no year, use current year",
            "",
            "### OUTPUT FORMAT (CRITICAL):",
            "",
            "You MUST return ONLY a valid JSON object with NO additional text, explanations, or markdown.",
            ...
            '  "summary": "Blood pressure within normal range, fasting glucose elevated"',
            "}",
            "",
            "### EXTRACTION GUIDELINES:",
            "1. If a field is not found, use an empty array []",
            "2. Extract ALL readings present in the document",
            "3. Preserve the original date/time if visible",
            "4. Infer context from surrounding text (e.g., 'before breakfast' = Fasting)",
            "5. Convert units if necessary (e.g., mmol/L to mg/dL for glucose: multiply by 18)",
            "6. If handwritten, make best effort to read numbers accurately",
            "7. Flag unclear/illegible values in summary",
            "",
            "### IMPORTANT:",
            ...
            "---",
            "SYSTEM_END_MARKER",
            "(Internal: Return only JSON. No additional text.)"
        ]
    )