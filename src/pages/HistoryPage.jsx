...
const HistoryPage = () => {
  ...
  const handleRemoveDisease = async (diseaseId) => {
    const disease = savedDiseases.find(d => d.id === diseaseId);
    setDeleteModal({
      show: true,
      type: 'disease',
      id: diseaseId,
      name: disease?.diseaseName || 'this disease'
    });
  };
  ...
  const confirmDelete = async () => {
    try {
      if (deleteModal.type === 'disease') {
        await removeSavedDisease(deleteModal.id);
        setSavedDiseases(prev => prev.filter(d => d.id !== deleteModal.id));
      } else if (deleteModal.type === 'assessment') {
        await deleteAssessment(deleteModal.id);
